...
class SmartShieldC2 {
  ...
  initWebSocket() {
    ...
    const customBackend = urlParams.get('backend') || localStorage.getItem('vayu_backend_url');

    let wsUrl;
    if (customBackend) {
      const clean = customBackend.trim().replace(/^https?:\/\//, '').replace(/^wss?:\/\//, '').replace(/\/$/, '');
      const isHttps = customBackend.startsWith('https') || window.location.protocol === 'https:';
      const wsProto = isHttps ? 'wss:' : 'ws:';
      const httpProto = isHttps ? 'https:' : 'http:';
      wsUrl = `${wsProto}//${clean}/ws/telemetry`;
      this.apiBase = `${httpProto}//${clean}`;
    } else {
    ...
    try {
      ...
      this.ws.onopen = () => {
        ...
        console.log("SMART SHIELD C2 WebSocket Connected to:", wsUrl);
        this.updateConnectionBadge(true);
        this.addEventLog(`[SYSTEM] Connected to C2 Live Telemetry Stream at 30 FPS (${wsUrl})`);
        
        // Refresh live camera feed
        const mjpegImg = document.getElementById('mjpegVideoFeed');
        if (mjpegImg && this.visionMode === 'LIVE_STREAM') {
          mjpegImg.src = `${this.apiBase || ''}/api/video_feed?t=${Date.now()}`;
